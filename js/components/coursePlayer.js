...
// renders it via lessonPlayer.js below the list.
import { renderLessonPlayer } from './lessonPlayer.js';
import { t } from '../i18n.js';

export function renderCoursePlayer(container, { supabase, course, onBack, onProgressChanged }) {
  ...
  function render(modules, lessons, completedSet, lockByLessonId) {
    container.innerHTML = `
      <button type="button" data-action="back" class="text-sm text-indigo-600 hover:text-indigo-800 mb-4">${t('courses.backToCatalog')}</button>
      ...
      <div data-el="lesson-panel" class="bg-white rounded-xl shadow p-4 sm:p-6"></div>
    `;
